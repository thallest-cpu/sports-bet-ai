...
# =============================================================
elif sport_choice == "🎯 Painel de Probabilidades":
    st.markdown("""
    <div class='betano-slide'>
        <div style='display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:14px;'>
            <div>
...
<div class='social-proof-bar'>
    <div style='display: flex; align-items: center; gap: 10px;'>
        ...
        <div>
            <span style='color: #10b981; font-weight: 900; font-size: 1.1em;'>TAXA DE ACERTO: {SOCIAL_PROOF_METRICS['win_rate']}%</span>
...
# =============================================================
# 🔴 TODOS OS JOGOS AO VIVO — FEED GLOBAL (fixtures?live=all)
# =============================================================
st.markdown("---")
st.header("🔴 TODOS OS JOGOS AO VIVO")

col_live_t1, col_live_t2 = st.columns([3, 1])
with col_live_t1:
    st.caption("Feed global em tempo real — placar, eventos e estatísticas atualizados automaticamente. Não é necessário selecionar liga.")
with col_live_t2:
    if st.button("🔄 Atualizar Jogos", use_container_width=True, key="btn_refresh_live"):
        st.rerun()

# Buscar TODOS os jogos ao vivo globalmente
jogos_global_live = get_api_football_all_live(api_key=active_fb_key)

if not jogos_global_live:
    st.info("⚽ Nenhuma partida ao vivo no momento. Volte mais tarde para acompanhar os jogos em tempo real!")
else:
    st.success(f"⚡ **{len(jogos_global_live)} partida(s) ao vivo agora** em todo o mundo")

    # Agrupar por competição
    jogos_por_liga = {}
    for jogo in jogos_global_live:
        league_info = jogo.get("league", {})
        liga_key = league_info.get("name", "Outra Competição")
        pais = league_info.get("country", "—")
        chave_grupo = f"{liga_key}|||{pais}"
        if chave_grupo not in jogos_por_liga:
            jogos_por_liga[chave_grupo] = []
        jogos_por_liga[chave_grupo].append(jogo)

    for grupo_key, jogos_do_grupo in jogos_por_liga.items():
        nome_liga, pais_liga = grupo_key.split("|||", 1)
        logo_liga = jogos_do_grupo[0].get("league", {}).get("logo", "")
        flag_liga = jogos_do_grupo[0].get("league", {}).get("flag", "")

        # Cabeçalho da competição
        header_parts = []
        if flag_liga:
            header_parts.append(f"<img src='{flag_liga}' width='20' style='vertical-align:middle; margin-right:4px;'/>")
        if logo_liga:
            header_parts.append(f"<img src='{logo_liga}' width='20' style='vertical-align:middle; margin-right:6px;'/>")
        header_parts.append(f"<b style='color:#ffd200; font-size:1.1em;'>{nome_liga}</b>")
        header_parts.append(f"<span style='color:#94a3b8; font-size:0.9em; margin-left:8px;'>🌎 {pais_liga}</span>")

        st.markdown(f"""
        <div style='background:rgba(255,91,0,0.08); border-left:3px solid #ff5b00; padding:8px 14px; margin:16px 0 8px 0; border-radius:4px;'>
            {''.join(header_parts)}
            <span style='color:#94a3b8; font-size:0.85em; float:right;'>({len(jogos_do_grupo)} jogo{'s' if len(jogos_do_grupo) > 1 else ''})</span>
        </div>
        """, unsafe_allow_html=True)

        for jogo in jogos_do_grupo:
            fixture_info = jogo.get("fixture", {})
            fixture_id = fixture_info.get("id", 0)
            status_info = fixture_info.get("status", {})
            minuto = status_info.get("elapsed", "—")
            status_short = status_info.get("short", "—")
            status_long = status_info.get("long", "Ao Vivo")

            teams_info = jogo.get("teams", {})
            home_team = teams_info.get("home", {})
            away_team = teams_info.get("away", {})
            casa = home_team.get("name", "Casa")
            fora = away_team.get("name", "Fora")
            logo_casa = home_team.get("logo", "")
            logo_fora = away_team.get("logo", "")

            goals_info = jogo.get("goals", {})
            gols_casa = goals_info.get("home", 0) if goals_info.get("home") is not None else 0
            gols_fora = goals_info.get("away", 0) if goals_info.get("away") is not None else 0

            # Badge de status
            if status_short in ("1H", "2H", "ET", "LIVE"):
                badge_status = f"🔴 {minuto}' AO VIVO"
                badge_color = "#ef4444"
            elif status_short == "HT":
                badge_status = "⏸️ INTERVALO"
                badge_color = "#f59e0b"
            elif status_short == "P":
                badge_status = "🎯 PÊNALTIS"
                badge_color = "#a855f7"
            elif status_short == "BT":
                badge_status = "⏸️ INTERVALO PRORROGAÇÃO"
                badge_color = "#f59e0b"
            else:
                badge_status = f"🔴 {status_long}"
                badge_color = "#ef4444"

            # Card da partida
            logo_home_html = f"<img src='{logo_casa}' width='28' style='vertical-align:middle; margin-right:6px;'/>" if logo_casa else ""
            logo_away_html = f"<img src='{logo_fora}' width='28' style='vertical-align:middle; margin-left:6px;'/>" if logo_fora else ""

            st.markdown(f"""
            <div class='betano-card' style='display:flex; justify-content:space-between; align-items:center; border-left:4px solid {badge_color}; margin-bottom:4px;'>
                <div>
                    <span style='font-size:1.2em; font-weight:800; color:#fff;'>
                        {logo_home_html}<b>{casa}</b>
                        <span class='badge-superodds' style='margin:0 8px;'>{gols_casa} x {gols_fora}</span>
                        <b>{fora}</b>{logo_away_html}
                    </span>
                </div>
                <span style='background:rgba(239,68,68,0.2); color:{badge_color}; padding:4px 10px; border-radius:6px; font-weight:700; font-size:0.85em; white-space:nowrap;'>
                    {badge_status}
                </span>
            </div>
            """, unsafe_allow_html=True)

            # Expander de eventos e estatísticas
            with st.expander(f"📡 Mostrar eventos e estatísticas ao vivo — {casa} vs {fora}", expanded=False):
                col_ev, col_st = st.columns(2)

                # ===== EVENTOS =====
                with col_ev:
                    st.markdown("##### ⚽ Eventos ao Vivo")
                    eventos = get_api_football_fixture_events(fixture_id, api_key=active_fb_key)
                    if eventos:
                        for ev in eventos:
                            ev_time = ev.get("time", {}).get("elapsed", "—")
                            ev_extra = ev.get("time", {}).get("extra")
                            time_str = f"{ev_time}'+{ev_extra}'" if ev_extra else f"{ev_time}'"
                            ev_team = ev.get("team", {}).get("name", "—")
                            ev_player = ev.get("player", {}).get("name", "—")
                            ev_assist = ev.get("assist", {}).get("name", "")
                            ev_type = ev.get("type", "")
                            ev_detail = ev.get("detail", "")

                            # Ícone por tipo de evento
                            if ev_type == "Goal":
                                if "Own Goal" in ev_detail:
                                    icon = "🔴⚽"
                                    desc = "Gol Contra"
                                ...
                                if "Yellow" in ev_detail:
                                    ...
                                    icon = "🟨"
                                    desc = ev_detail or "Cartão"
                            elif ev_type == "subst":
                                icon = "🔄"
                                desc = "Substituição"
                                ev_player_display = f"↗️ {ev_assist}" if ev_assist else ""
                                player_out = ev_player or ""
                                st.markdown(f"**{time_str}** {icon} `{ev_team}` — {desc}: ↘️ {player_out} {ev_player_display}")
                                continue
                            elif ev_type == "Var":
                                icon = "📺"
                                desc = f"VAR — {ev_detail}" if ev_detail else "VAR"
                            else:
                                icon = "📋"
                                desc = ev_detail or ev_type or "Evento"

                            assist_txt = f" (assist: {ev_assist})" if ev_assist and ev_type == "Goal" else ""
                            st.markdown(f"**{time_str}** {icon} `{ev_team}` — {desc}: **{ev_player}**{assist_txt}")
                    else:
                        st.caption("Nenhum evento registrado até o momento nesta partida.")

                # ===== ESTATÍSTICAS =====
                with col_st:
                    st.markdown("##### 📊 Estatísticas ao Vivo")
                    stats_raw = get_api_football_fixture_statistics(fixture_id, api_key=active_fb_key)
                    if stats_raw and len(stats_raw) >= 2:
                        stats_home_list = stats_raw[0].get("statistics", [])
                        stats_away_list = stats_raw[1].get("statistics", [])

                        # Tradução dos nomes de estatísticas
                        stat_translation = {
                        ...
                        # Montar dict de stats para cada time
                        home_stats = {s.get("type", ""): s.get("value") for s in stats_home_list}
                        away_stats = {s.get("type", ""): s.get("value") for s in stats_away_list}

                        all_stat_keys = list(dict.fromkeys(
                            [s.get("type", "") for s in stats_home_list] +
                            [s.get("type", "") for s in stats_away_list]
                        ))

                        rows = []
                        for sk in all_stat_keys:
                            h_val = home_stats.get(sk)
                            a_val = away_stats.get(sk)
                            nome_pt = stat_translation.get(sk, sk)
                            h_display = str(h_val) if h_val is not None else "—"
                            a_display = str(a_val) if a_val is not None else "—"
                            rows.append({"🏠 Casa": h_display, "📊 Métrica": nome_pt, "🏟️ Fora": a_display})

                        if rows:
                            df_stats = pd.DataFrame(rows)
                            st.dataframe(df_stats, use_container_width=True, hide_index=True)
                        else:
                            st.caption("Estatísticas não disponíveis para esta partida.")
                    else:
                        st.caption("Estatísticas não disponíveis para esta partida.")

# 2. Todos os Jogos de Hoje (liga selecionada — preservado)
...
with tab_table:
    ...
    # Mapear o nome curto da liga (sidebar) para o nome completo com temporada
    _league_season_key = API_LEAGUE_TO_SEASON_KEY.get(selected_league, None)
    _league_keys_list = list(LEAGUE_CODES_CURRENT_SEASON.keys())
    _default_table_index = _league_keys_list.index(_league_season_key) if _league_season_key in _league_keys_list else 0

    table_league = st.selectbox(
        "Escolha o Campeonato para Visualizar a Tabela:",
        options=_league_keys_list,
        index=_default_table_index,
